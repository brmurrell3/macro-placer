"""E124 — Per-net-trace differentiable RUDY proxy WITH phantom-stripe fix.

Forked from E111's `per_net_trace_proxy.py`. The only behavioral change
is in `_trace_route_congestion`: when computing the range endpoints
[col_min, col_max] and [row_min, row_max], the original code used
`_soft_min_max` (log-sum-exp soft-min / soft-max with β=6). For
degenerate inputs (col_src == col_snk, e.g., source and sink in the
same column), soft_min_max produces:
    soft_min = a - log(2)/β,  soft_max = a + log(2)/β
i.e., a phantom stripe of width ~2*log(2)/β ≈ 0.23 cells (with β=6).

Canonical RUDY assigns ZERO H-stripe width when source col == sink col.
The phantom stripe creates fake demand mass in cells that don't actually
carry routing.

The fix mirrors E121 MCF's approach: use `torch.minimum`/`torch.maximum`
on the soft-expected endpoint indices. This is exact (no phantom), but
because the inputs (col_src_exp, col_snk_exp) are themselves smooth
expectations of softmax over cells, the overall function remains
differentiable w.r.t. positions almost everywhere (subgradient at the
equality case, same as any min/max in PyTorch). Range indicators
downstream (sigmoid steepness β_range) still smooth the stripe width.

This is a DROP-IN replacement for `PerNetTraceCongestion` from E111.
Same constructor signature, same compute_congestion API.
"""
...
_spec = importlib.util.spec_from_file_location("_e124_dpo", str(_DPO_PATH))
...
class PerNetTraceCongestionFixed:
    """Differentiable per-net L-route congestion proxy with phantom-stripe fix.

    See module docstring; structurally identical to E111
    `PerNetTraceCongestion` except for the (col_min, col_max) /
    (row_min, row_max) computation.
    """
    ...
    def _exact_min_max(
        ...
        """Exact (min, max) of (a, b) using torch.minimum/maximum.

        This is the phantom-stripe FIX. The original E111 _soft_min_max
        used soft-min/soft-max via logsumexp, which creates a
        ~2*log(2)/β ≈ 0.23-cell phantom stripe at degenerate pairs
        (a == b). torch.minimum/maximum give exact zero width, while
        still flowing gradient back through whichever argument is
        smaller/larger (standard subgradient at the tie).
        """
    ...
    def _trace_route_congestion(
        ...
        for b_start in range(0, n_pairs, cs):
            ...
            # PHANTOM-STRIPE FIX: exact min/max instead of soft-min/soft-max.
            # Eliminates spurious ~0.23-cell stripe at degenerate (col_src == col_snk)
            # or (row_src == row_snk) pairs.
            col_min, col_max = self._exact_min_max(col_src_exp, col_snk_exp)
            row_min, row_max = self._exact_min_max(row_src_exp, row_snk_exp)
