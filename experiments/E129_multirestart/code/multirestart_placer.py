"""E129 — Multi-restart best-of-N for thinkorplace-v2.

Run N seeds of V4+Gaussian descent + legalize, score each by canonical
proxy, pick best, then run CD polish on the best basin.

Exploits FP32 nondeterminism across seeds (each seed finds a slightly
different basin) to get the best of N samples.

Pipeline per bench:
  for seed in [42, 142, 242]:
    pos = V4+Gauss descent + legalize
    if ovl == 0:
      cand[seed] = (canonical_proxy(pos), pos)
  best = min(cand, key=proxy)
  return CD_polish(best)

Total per-bench wall: N*(descent + legalize) + 1*CD_polish
At N=3: 3*120s + 600s = 960s = 16 min/bench (still under 60 min cap)
17 benches serial: 4.5 hr; 4-way parallel: ~1.5 hr
"""
...
class E129MultiRestartPlacer:
    """thinkorplace-v2 with multi-restart best-of-N descent."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        self._log(f"=== E129 multi-restart ({benchmark.name}) N={self.n_restarts} ===")
        ...
        for i, seed in enumerate(self.seeds[: self.n_restarts]):
            ...
            # Budget safety
            if deadline is not None and time.time() > deadline - 700:
        ...
        if not results:
            ...
            best_proxy, best_seed, pos = results[0]
            self._log(f"  BEST seed {best_seed}: basin={best_proxy:.5f}")

        # Phase 2: CD polish on best basin
