"""E123 — V3 placer with Sinkhorn-relaxed top-K congestion.

Drop-in replacement for `SmoothGlobalPlacerV3` (E111) that uses
`SinkhornPerNetTraceCongestion` instead of `PerNetTraceCongestion`.
Only the proxy class differs; all hparams (Adam descent, γ-anneal, overlap
λ ramp, legalize) are unchanged so this is a fair A/B vs E111 V3.
"""
...
from sinkhorn_per_net_trace import SinkhornPerNetTraceCongestion  # noqa: E402


class DiffProxyV3Sinkhorn(DiffProxyV2):
    """DiffProxyV2 with SinkhornPerNetTraceCongestion replacing _rudy_congestion."""
...
def loss_with_penalty_v3_sinkhorn(
    proxy: DiffProxyV3Sinkhorn,
    ...
    *,
    ...
    """Mirror E111 loss_with_penalty_v3 but use the sinkhorn proxy."""
...
class SmoothGlobalPlacerV3Sinkhorn:
    """Mirrors SmoothGlobalPlacerV3 but with Sinkhorn-relaxed top-K congestion.

    Differs from E111 V3 in exactly one way: the congestion proxy uses
    Sinkhorn instead of torch.topk. All other hparams (steps, lr, γ, overlap λ,
    legalize, init) are identical, so this is a fair A/B test.
    """
    ...
    def __init__(
        ...
        overlap_lambda_end: float = 10.0,  # V3Min ovl10 baseline value
        ...
        # ε annealing: linearly transition from start → end during descent
        sinkhorn_eps_end: Optional[float] = None,
    ):
        ...
        self.sinkhorn_eps_end = sinkhorn_eps_end
    ...
    def descend(
        ...
        for step in range(num_steps):
            ...
            # Sinkhorn ε anneal: if eps_end is set, linearly transition ε.
            if self.sinkhorn_eps_end is not None:
                eps_now = self.sinkhorn_eps + t * (self.sinkhorn_eps_end - self.sinkhorn_eps)
                proxy.trace.sinkhorn_eps = eps_now
            ...
            optimizer.zero_grad()
            loss, parts = loss_with_penalty_v3_sinkhorn(
            ...
            if step % self.log_every == 0 or step == num_steps - 1:
                eps_str = f" ε={proxy.trace.sinkhorn_eps:.3f}" if self.sinkhorn_eps_end is not None else ""
                self._log(
                    ...
                    f"γ={gamma_frac:.4f}  λ_ovl={overlap_lambda:.1f}{eps_str}"
                )
                history.append({
                    "step": step,
                    "loss": last_loss,
                    ...
                    "sinkhorn_eps": proxy.trace.sinkhorn_eps,
                })
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacerV3Sinkhorn ({benchmark.name}) ===")
        eps_str = (
            f"{self.sinkhorn_eps}"
            if self.sinkhorn_eps_end is None
            else f"{self.sinkhorn_eps}→{self.sinkhorn_eps_end}"
        )
        self._log(
            ...
            f"device={self.device} sinkhorn={self.use_sinkhorn} ε={eps_str} "
            f"iters={self.sinkhorn_iters}"
        )
