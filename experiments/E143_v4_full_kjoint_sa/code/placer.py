"""E143 — E142 + K-joint LNS + SA polish v2.

Full Option C architecture on V4 base, extended with K-joint LNS (E39)
and SA polish v2 (E25) as final polish phases.

Pipeline:
  1. Multi-init multi-seed (sdf42, sdf123, sdf999, dpo42)
  2. CD polish
  3. Cascade saddle escape
  4. Portfolio saddle escape
  5. K-joint LNS (K=3)
  6. SA polish v2
...
class Placer:
    """Full stack: V4 multi-init/seed + cascade + portfolio + K-joint + SA."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        self._log(f"=== E143 v4_full_kjoint_sa ({benchmark.name}) budget={self.budget_seconds}s ===")
        ...
        try:
            ...
            kstats = run_kjoint_lns(
                ...
                K=self.kjoint_K, top_N=self.kjoint_top_N,
            ...
            if ko == 0 and kp < current_proxy - 1e-7:
                self._log(f"  K-joint: {current_proxy:.5f} -> {kp:.5f} ({100*(current_proxy-kp)/current_proxy:+.2f}%) tried={kstats.get('ktuples_tried', 0)} committed={kstats.get('ktuples_committed', 0)}")
                current = kpos
                current_proxy = kp
            else:
        ...
        try:
            ...
            if so == 0 and sp < current_proxy - 1e-7:
                self._log(f"  SA: {current_proxy:.5f} -> {sp:.5f} ({100*(current_proxy-sp)/current_proxy:+.2f}%) accepted={sa_stats.get('accepted_better', 0)}/{sa_stats.get('proposed', 0)}")
                current = spos
                current_proxy = sp
            else:
