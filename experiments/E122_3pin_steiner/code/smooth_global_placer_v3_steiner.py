"""E122 SmoothGlobalPlacerV3Steiner — V3 with E122 3-pin Steiner congestion.

Drop-in variant of `experiments/E111_per_net_trace_congestion/code/
smooth_global_placer_v3.py` that swaps `PerNetTraceCongestion` for
`PerNetTraceCongestionSteiner3Pin` (E122). Everything else (LSE-HPWL,
grid density, γ-anneal, overlap penalty ramp, boundary penalty, init,
optimizer, legalize) is identical.

Hypothesis: 3-pin Steiner T-route gives Adam a more canonical-aligned
gradient on the 25-40% of nets that are 3-pin, producing a basin closer
to canonical optimum on hard benches (ibm17).
"""
...
from three_pin_steiner import PerNetTraceCongestionSteiner3Pin  # noqa: E402


class DiffProxyV3Steiner(DiffProxyV2):
    """DiffProxyV2 with PerNetTraceCongestionSteiner3Pin (E122) replacing _rudy_congestion."""
    ...
    def __init__(
        ...
        self.trace = PerNetTraceCongestionSteiner3Pin(
...
def loss_with_penalty_v3s(
    proxy: DiffProxyV3Steiner,
    ...
    *,
    ...
    """Mirror E88's loss_with_penalty but use DiffProxyV3Steiner (E122 congestion)."""
...
class SmoothGlobalPlacerV3Steiner:
    """E110 SmoothGlobalPlacer + E122 3-pin Steiner congestion.

    Same args as E111 V3. The only behavioral difference is `cost()` uses
    `PerNetTraceCongestionSteiner3Pin` (E122) instead of the plain E111
    star-L-route proxy.
    """
    ...
    def descend(
        ...
        for step in range(num_steps):
            ...
            optimizer.zero_grad()
            loss, parts = loss_with_penalty_v3s(
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacerV3Steiner ({benchmark.name}) ===")
        self._log(
