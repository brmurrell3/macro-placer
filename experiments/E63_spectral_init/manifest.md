...
outcome: **implementation V2/V3 in progress 2026-05-02 → 03 — both legalizers hit boundary issues with fixed macros.** Hungarian-on-slot-grid (V2): solves in 0.02s but ibm01 fits only 28 max-size slots vs 246 hard movables; finer fallback grid violates no-overlap-by-construction → 9-36 residuals. Row-packing (V3): correct sort-and-pack-rows-L→R algorithm, but doesn't avoid FIXED macros pre-placed on canvas → 111 residuals on ibm01 from movable↔fixed overlaps. **Both algorithms work in principle for movable-only sub-problems.** What's needed: a legalizer that (a) row-packs movables AROUND fixed macros (subtract fixed regions from row available x-spans), or (b) post-Hungarian/row-pack greedy displacement of movables that overlap fixed macros to nearest free spot. Defer to next session. **NEB cross-section (E65) is the more immediate diagnostic** — consumes E25/E41 basins directly, delivers overnight.
