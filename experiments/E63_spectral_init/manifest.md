...
outcome: **implementation incomplete 2026-05-02 22:13** — spectral init computes Laplacian eigenvectors successfully (~0.2 s on ibm01, eigenvalues 1e-9, 0.219, 0.358, 0.363) but legalization is non-trivial. Three approaches tried: (a) **direct linear rescaling + project_overlaps** → 270 residual overlaps after 50 iters because spectral coords cluster connected macros; (b) **rank-based mapping + greedy_legalize** → ran 2+ hr on ibm01 without completing because greedy_legalize is O(N³) when many macros conflict at canvas-fraction-of-N spacing; (c) **SDF baseline + spectral permutation** → 78 overlaps because permuting macros across SDF positions ignores per-macro size constraints (macro A's slot may not fit macro B). **The conceptual mechanism is sound but legalization needs proper design — likely Hungarian assignment with size-aware cost, or a custom row-based legal placer that respects spectral ordering.** Estimated additional dev: 2-4 hours focused work. Deferred until after E64 LP-bounded beam K-joint scaffolding.
