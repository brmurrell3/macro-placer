"""E111 — SmoothGlobalPlacerV3: E110 + per-net-trace congestion (E111).

Drop-in replacement for E110's SmoothGlobalPlacer with the bbox-uniform
`_rudy_congestion` swapped for `PerNetTraceCongestion`. Everything else
(LSE-HPWL, grid density, γ-anneal, overlap penalty ramp, boundary
penalty, init, optimizer, legalize) is identical to E110.

Hypothesis: the canonical-matching congestion gives Adam a more
canonical-aligned gradient on hard benches (ibm17 above all), enabling
a basin closer to what cascade polish exploits.

Validation budget: ibm17 single-bench, target raw < 1.50 + CD60s < 1.40.
"""
...
class DiffProxyV3(DiffProxyV2):
    """DiffProxyV2 with PerNetTraceCongestion replacing _rudy_congestion."""
    ...
    def cost(self, positions: torch.Tensor, *, include_congestion: bool = True):
        ...
        # WL and density: reuse DiffProxyV2's primitives
...
def loss_with_penalty_v3(
    ...
    *,
    ...
    """Mirror E88's loss_with_penalty but use DiffProxyV3 (E111 congestion)."""
...
class SmoothGlobalPlacerV3:
    """E110 SmoothGlobalPlacer + E111 per-net-trace congestion.

    Same constructor args as SmoothGlobalPlacer. The only behavioral
    difference is `cost()` uses `PerNetTraceCongestion` instead of the
    DPO-era bbox-uniform `_rudy_congestion`.
    """
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacerV3 ({benchmark.name}) ===")
        self._log(
