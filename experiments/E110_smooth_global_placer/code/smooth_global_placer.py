"""E110 — Adam descent on E95 DiffProxy + greedy legalize.

The cascade is basin-saturated. Gradient-based descent on the smooth
proxy (E88 LSE-HPWL + grid density + ABU-5% RUDY congestion, with E95's
γ-anneal + WL normalization fix) is a fundamentally different basin
generator. Output is legalized with greedy_macro_legalize and intended
as Lane 4 in stacked_periphery plateau-pick.

Pure PyTorch (autograd on CPU or MPS); macro-only problem size makes
Triton kernels unnecessary.
"""
...
from diff_proxy_v2 import DiffProxyV2, loss_with_penalty  # noqa: E402
from macro_legalizer import greedy_macro_legalize  # noqa: E402


class SmoothGlobalPlacer:
    """Adam descent on the smooth proxy, then legalize.

    Designed as a lane-4 init for stacked_periphery. Returns a legal
    (zero-overlap) Tensor placement that the cascade can polish from.
    """
    ...
    def _init_positions(self, benchmark: Benchmark, plc=None) -> torch.Tensor:
        if self.init == "sdf":
            ...
            assert plc is not None, "DPO init requires plc"
    ...
    def descend(
        ...
        """Run the Adam descent only. Returns (positions, stats).

        Positions are NOT legalized yet (may have overlaps). Stats has
        per-step trajectory in 'history' (sampled) and final 'cost'/'parts'.
        """
        t0 = time.time()
        ...
        # Resolve device, with MPS fallback to CPU if not available.
    ...
    def legalize(
        ...
        """Greedy spiral legalize, with project_overlaps fallback if needed."""
        t0 = time.time()
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacer ({benchmark.name}) ===")
        self._log(
