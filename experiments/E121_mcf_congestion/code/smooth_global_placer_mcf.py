"""E121 — SmoothGlobalPlacerMCF: V3 + MCF congestion (drop-in for V3).

Mirrors E111 SmoothGlobalPlacerV3 but swaps PerNetTraceCongestion for
MCFCongestion. Hyperparameters (steps, lr_frac, gamma, overlap_lambda)
are unchanged so the only behavioral difference is the smooth congestion
proxy used inside DiffProxy.cost().
"""
...
from mcf_congestion import MCFCongestion  # noqa: E402


class DiffProxyMCF(DiffProxyV2):
    """DiffProxyV2 with MCFCongestion replacing _rudy_congestion."""
    ...
    def __init__(
        ...
        mcf_kwargs: Optional[Dict] = None,
    ):
        super().__init__(benchmark, plc, device=device, gamma_frac=gamma_frac)
        mcf_kwargs = mcf_kwargs or {}
        self.mcf = MCFCongestion(
            benchmark, plc, device=device, **mcf_kwargs
        )
    ...
    def cost(self, positions: torch.Tensor, *, include_congestion: bool = True):
        ...
        if include_congestion:
            cong = self.mcf.compute_congestion(clamped)
        else:
...
class SmoothGlobalPlacerMCF:
    """V3 placer using MCFCongestion (multi-commodity flow proxy)."""
    ...
    def descend(
        ...
        if self.adaptive_num_steps:
            ...
            num_steps = max(200, int(n_macros * self.steps_per_macro))
            self._log(f"  adaptive_num_steps: → {num_steps}")
        ...
        for step in range(num_steps):
            ...
            optimizer.zero_grad()
            loss, parts = loss_with_penalty_mcf(
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacerMCF ({benchmark.name}) ===")
        self._log(
