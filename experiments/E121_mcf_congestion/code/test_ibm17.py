"""E121 — Smoke test on ibm17 with MCF congestion (vs V3 / Lane-4 baseline).

Compares:
  1. E111 SmoothGlobalPlacerV3 (per-net trace; RUDY topology)  — baseline
  2. E121 SmoothGlobalPlacerMCF (multi-commodity flow)         — variant

Then CD600s polish on each.

Success: MCF raw < 1.50, MCF + CD600s < 1.20 (V3Min ovl10 720s baseline).
"""
...
from smooth_global_placer_mcf import SmoothGlobalPlacerMCF  # noqa: E402
...
def main():
    ...
    parser.add_argument("--cd_budget", type=float, default=CD_BUDGET_S)
    parser.add_argument("--tau", type=float, default=0.05)
    parser.add_argument("--rudy_mix", type=float, default=1.0)
    parser.add_argument("--nbend", type=int, default=3)
    ...
    print(f"Loaded {args.bench}: {benchmark.num_macros} macros "
    ...
    cfg = dict(num_steps=500, lr_frac=0.005, init="sdf",
               overlap_lambda_end=10.0,  # match thinkorplace-v2 default
               verbose=True, log_every=100)

    results = []

    if not args.skip_v3:
        placer_v3 = SmoothGlobalPlacerV3(**cfg)
        results.append(run_one("E111 V3 (per-net trace, RUDY-topology)", placer_v3, benchmark, plc))
        benchmark, plc = load_benchmark_from_dir(str(bench_dir))

    placer_mcf = SmoothGlobalPlacerMCF(
        **cfg,
        mcf_kwargs=dict(tau_frac=args.tau, rudy_mix=args.rudy_mix, n_bend_samples=args.nbend),
    )
    results.append(run_one(
        f"E121 MCF (τ={args.tau} rudy_mix={args.rudy_mix} K={args.nbend})",
        placer_mcf, benchmark, plc,
    ))
    ...
    print(f"\nBaseline: thinkorplace-v2 ibm17 (V3 + CD600s) ~ 1.20", flush=True)

    out_path = _HERE.parent / "results" / "ibm17_mcf_smoke.json"
