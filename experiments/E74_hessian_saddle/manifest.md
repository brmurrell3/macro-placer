...
Henkelman & Jónsson 2000 (climbing-image NEB) and the broader
transition-state-theory literature establish the mathematical
foundation: at a local minimum of a smooth potential, the Hessian's
smallest eigenvector points along the softest mode, and following
that mode for ε crosses to a neighboring basin separated by a saddle
of bounded height. This experiment applies that mechanism to the
local-move plateau of the macro placement proxy.
...
- Roadmap E28 (never built until this experiment):
  `docs/roadmap.md` §5.1 Tier 0.
- Henkelman & Jónsson 2000, "Climbing image NEB", J. Chem. Phys. — the
  foundational transition-state method our mechanism applies.
