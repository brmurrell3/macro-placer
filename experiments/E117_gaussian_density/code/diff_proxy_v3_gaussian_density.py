"""E117 — DiffProxyV3GaussianDensity: V3 proxy + Gaussian-smoothed density.

The default `_grid_density` (used by DiffProxyV3) is piecewise-linear with
step discontinuities at cell boundaries — bad for Adam descent on dense
benches (ibm12/14/17/18). This swaps in a C^inf smooth Gaussian-smeared
density model (erf-based per-cell integration) while keeping V3's
per-net-trace congestion and LSE-HPWL intact.

Drop-in compatible with E111Minimal/V3Min pipelines via the
`SmoothGlobalPlacerV3GaussianDensity` driver below.

Key design choice: the density model is FOR THE DESCENT GRADIENT ONLY.
The canonical evaluation (PlacementCost.get_density_cost) is untouched.
We're trading off some smooth-vs-canonical alignment for a continuous
gradient that Adam can actually follow on dense benches.
"""
...
from smooth_global_placer_v3 import DiffProxyV3  # noqa: E402
...
from gaussian_density import GaussianGridDensity  # noqa: E402


class DiffProxyV3GaussianDensity(DiffProxyV3):
    """V3 (per-net trace congestion) + Gaussian-smoothed density.

    Parameters
    ----------
    sigma_scale : float
        Multiplier on half-macro-size used as Gaussian sigma. 1.0 = exact
        half-macro-size (so 1-sigma covers half the macro). Larger ⇒
        broader smear; smaller ⇒ closer to hard rectangle (less smoothing
        benefit, but closer to canonical scale).
    sigma_floor_frac : float
        Minimum sigma per axis as a fraction of cell_w/cell_h. Prevents
        tiny macros from becoming delta-spikes (which would give 0
        gradient outside their cell).
    """
    ...
    def cost(self, positions: torch.Tensor, *, include_congestion: bool = True):
        """Smooth proxy. WL + 0.5*density + 0.5*trace_congestion.

        Density is now Gaussian-smeared (erf-based, C^inf smooth).
        WL and congestion match parent V3.
        """
        if positions.device != self.device:
            positions = positions.to(self.device)
        clamped = self._clamp_to_canvas(positions)
        # WL: reuse parent's primitives
        from diff_proxy import _lse_hpwl  # noqa: E402
        wl = _lse_hpwl(clamped, self.net_data, self.port_base, self.gamma) / self.wl_norm
        # Density: SWAP in Gaussian
...
def loss_with_penalty_v3_gaussian(
    ...
    *,
    ...
    """Like loss_with_penalty_v3 but typed for the Gaussian proxy."""
...
class SmoothGlobalPlacerV3GaussianDensity:
    """V3 + Gaussian density driver.

    Identical interface to `SmoothGlobalPlacerV3`; just swaps in
    `DiffProxyV3GaussianDensity` for the smooth proxy.
    """
    ...
    def descend(self, benchmark: Benchmark, plc) -> Tuple[torch.Tensor, Dict]:
        ...
        for step in range(num_steps):
            ...
            optimizer.zero_grad()
            loss, parts = loss_with_penalty_v3_gaussian(
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        t0 = time.time()
        self._log(f"=== SmoothGlobalPlacerV3GaussianDensity ({benchmark.name}) ===")
        self._log(
            ...
            f"sigma_floor_frac={self.sigma_floor_frac}"
        )
