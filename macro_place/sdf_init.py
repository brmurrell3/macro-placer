"""
SDF-Based Soft Rasterization Placer — v5
...
class SDFPlacer:
    """SDF-based analytical placer v5 — tuned from v2."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        for step in range(self.n_iters):
            ...
            # Density: use top-10% penalty (matches actual metric)
            ...
            # Overlap (ramp up with sqrt schedule for faster initial response)
            ov_w = self.overlap_weight * (0.05 + 0.95 * frac ** 0.5)
            ov_loss = overlap_penalty(clamped, half_sizes) / (cw * ch)

            # Density weight: higher early for spreading, lower late for WL
            dw = self.density_weight * (1.5 - 0.8 * frac)

            loss = wl_loss + dw * density_loss + ov_w * ov_loss
            loss.backward()
            optimizer.step()
            scheduler.step()
