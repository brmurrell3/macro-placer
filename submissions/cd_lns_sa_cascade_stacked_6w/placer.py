"""CDLNSSACascadeStacked6WPlacer — cascade_saddle FOLLOWED BY portfolio_saddle.
...
class CDLNSSACascadeStacked6WPlacer:
    ...
    def __init__(
        ...
        # Six non-canonical weights — adds 3 single-component directions to
        # the 3-weight portfolio for more eigvec diversity. Canonical
        # (1, 0.5, 0.5) is handled by cascade_saddle.
        self.portfolio = [
            (1.0, 0.0, 1.0),  # cong-focus
            (1.0, 1.0, 0.0),  # density-focus
            (0.0, 1.0, 1.0),  # non-WL
            (1.0, 0.0, 0.0),  # WL-only
            (0.0, 1.0, 0.0),  # density-only
            (0.0, 0.0, 1.0),  # cong-only
        ]

    def place(self, benchmark: Benchmark) -> torch.Tensor:
        log = lambda s: print(s, flush=True) if self.verbose else None
        log(f"=== CDLNSSACascadeStacked6WPlacer ({benchmark.name}) ===")
        t0 = time.time()
