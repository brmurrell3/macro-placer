"""CDLNSSACascadeStackedDualseedPlacer — cascade_saddle FOLLOWED BY portfolio_saddle.
...
class CDLNSSACascadeStackedDualseedPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeStackedDualseedPlacer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        if portfolio_budget < 90.0:
            ...
            # Dualseed: run portfolio_saddle TWICE with different rng seeds.
            # Each gets half the budget. Pick canonical-min of the two.
            per_run_budget = portfolio_budget / 2.0
            log(f"  Phase 3b: dualseed portfolio_saddle (budget {portfolio_budget:.0f}s, "
                f"per-run={per_run_budget:.0f}s, K_eps={self.portfolio_K_eps})")
            best_p_state, best_p_proxy = cascade_state, cascade_proxy
            for run_seed in [self.rng_seed, self.rng_seed + 1]:
                try:
                    p_state, p_stats = portfolio_saddle_escape(
                        ...
                        total_budget_s=per_run_budget,
                        rng_seed=run_seed,
                        log=lambda s: None,
                    )
                    p_proxy = float(compute_proxy_cost(p_state, benchmark, plc)["proxy_cost"])
                    log(f"  portfolio seed={run_seed}: {p_proxy:.5f} "
                        f"iters={p_stats.get('iters_run', '?')}")
                    if p_proxy < best_p_proxy:
                        best_p_state, best_p_proxy = p_state, p_proxy
                except Exception as exc:
                    log(f"  portfolio seed={run_seed} failed: {exc}")
            portfolio_state, portfolio_proxy = best_p_state, best_p_proxy
            log(f"  portfolio dualseed best: {portfolio_proxy:.5f}")
