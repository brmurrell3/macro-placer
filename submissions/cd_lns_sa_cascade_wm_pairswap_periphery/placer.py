"""CDLNSSACascadeStackedWMPairSwapPeripheryPlacer — OC+WM + pair-swap polish.

Adds pair-swap polish layer AFTER WireMask. Each layer is conservative
(only accept if proxy strictly improves AND zero overlaps).

Layers (in order):
  1. Stacked (E25 + E41 → plateau → cascade saddle → portfolio saddle)
  2. WireMask greedy reposition polish (criticality-ranked 2D atomic moves)
  3. NEW: Pair-swap polish (atomic 2-macro swaps; criticality-biased pairs)
  4. Periphery push + CD polish (strict-conservative accept).
"""
...
_PAIRSWAP_CODE = _ROOT / "experiments" / "E114_pair_swap" / "code"
if str(_PAIRSWAP_CODE) not in sys.path:
    sys.path.insert(0, str(_PAIRSWAP_CODE))
...
class CDLNSSACascadeStackedWMPairSwapPeripheryPlacer:
    """Option C + WireMask + pair-swap polish layers."""
    ...
    def __init__(
        ...
        pairswap_n_pairs_per_pass: int = 500,
        pairswap_max_passes: int = 3,
        pairswap_budget_s: float = 90.0,
        verbose: bool = True,
    ):
        ...
        self.pairswap_n_pairs_per_pass = pairswap_n_pairs_per_pass
        self.pairswap_max_passes = pairswap_max_passes
        self.pairswap_budget_s = pairswap_budget_s
        self.verbose = verbose

    def place(self, benchmark: Benchmark) -> torch.Tensor:
        log = lambda s: print(s, flush=True) if self.verbose else None
        log(f"=== CDLNSSACascadeStackedWMPairSwapPeripheryPlacer ({benchmark.name}) ===")
        t0 = time.time()
        deadline = t0 + self.budget_seconds if self.budget_seconds else None

        # Budget split: reserve wiremask + pairswap + periphery.
        reserved = (
            self.wiremask_budget_s + self.pairswap_budget_s
            + self.periphery_polish_s + 30.0
        )
        stacked_budget = (
        ...
        log(f"  budget split: stacked={stacked_budget}s, wiremask={self.wiremask_budget_s}s, "
            f"pairswap={self.pairswap_budget_s}s, periphery={self.periphery_polish_s}s")
        ...
        # ── Pair-swap polish layer ───────────────────────────────────────
        if deadline is not None:
            remaining = deadline - time.time()
            ps_budget = min(self.pairswap_budget_s,
                            remaining - self.periphery_polish_s - 30.0)
        else:
            ps_budget = self.pairswap_budget_s

        if ps_budget < 30.0:
            log(f"  SKIPPING pair_swap (only {ps_budget:.0f}s left)")
        else:
            try:
                log(f"  Pair-swap polish (budget {ps_budget:.0f}s, "
                    f"n_pairs={self.pairswap_n_pairs_per_pass}, "
                    f"max_passes={self.pairswap_max_passes})")
                fixed = benchmark.macro_fixed.cpu().numpy()
                hard_movable = [i for i in range(benchmark.num_hard_macros)
                                if not bool(fixed[i])]
                ev_ps = IncrementalProxyEvaluator(
                    benchmark, plc, current_placement.clone()
                )
                ps_stats = pair_swap_polish(
                    ev_ps, benchmark, hard_movable,
                    n_pairs_per_pass=self.pairswap_n_pairs_per_pass,
                    max_passes=self.pairswap_max_passes,
                    seed=self.rng_seed,
                    pair_selection="criticality",
                    time_budget_s=ps_budget,
                    log=lambda s: None,
                )
                ps_placement = ev_ps.placement.detach().clone().to(torch.float32)
                ps_proxy = float(compute_proxy_cost(ps_placement, benchmark, plc)["proxy_cost"])
                ps_ovl = int(compute_overlap_metrics(ps_placement, benchmark)["overlap_count"])
                log(f"  pair_swap done: proxy={ps_proxy:.5f} ovl={ps_ovl} "
                    f"Δ={ps_proxy-current_proxy:+.5f} "
                    f"({100*(ps_proxy-current_proxy)/current_proxy:+.3f}%) "
                    f"(wall={time.time() - t0:.0f}s)")
                if ps_ovl == 0 and ps_proxy < current_proxy - 1e-7:
                    log(f"  ACCEPT pair_swap: {current_proxy:.5f} → {ps_proxy:.5f}")
                    current_placement = ps_placement
                    current_proxy = ps_proxy
                else:
                    log(f"  REJECT pair_swap (ovl={ps_ovl} or no improvement)")
            except Exception as exc:
                log(f"  pair_swap FAILED: {exc}; keeping current")
...
Placer = CDLNSSACascadeStackedWMPairSwapPeripheryPlacer
