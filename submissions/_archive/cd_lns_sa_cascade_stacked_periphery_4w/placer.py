...
_STACKED_PATH = _ROOT / "submissions" / "cd_lns_sa_cascade_stacked_4w" / "placer.py"
_spec = importlib.util.spec_from_file_location("stacked_placer_4w", str(_STACKED_PATH))
_stacked = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_stacked)
CDLNSSACascadeStacked4WPlacer = _stacked.CDLNSSACascadeStacked4WPlacer
...
class CDLNSSACascadeStackedPeriphery4WPlacer:
    ...
    def __init__(
        ...
        portfolio_max_iters: int = 1,        # 6w: 1 iter with 6 weights
        portfolio_K_eps: int = 1,            # 6w: K=1 to fit budget
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeStackedPeriphery4WPlacer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        stacked = CDLNSSACascadeStacked4WPlacer(
