"""E115 SmoothGlobalPlacerV4 — V3 with FastDiffProxy backbone.

Verbatim copy of V3's `descend` / `legalize` / `place` flow with
`DiffProxyV3` swapped for `FastDiffProxy`. The only behavioral difference
is internal: faster CUDA per-step via index_select gather + single-pass
trace congestion. No hyperparameter changes.

Verified numerical equivalence: same proxy values to fp32 (see
bench_fast.py). Verified speedup: 16.85× per Adam step on ibm17 CUDA.
"""
...
from fast_proxy import FastDiffProxy, fast_loss_with_penalty  # noqa: E402


class SmoothGlobalPlacerV4:
    """E110 SmoothGlobalPlacer with FastDiffProxy (E115 acceleration).

    Same constructor args / behavior as V3 — only the proxy implementation
    differs (CUDA: 16.85× faster per step).
    """
