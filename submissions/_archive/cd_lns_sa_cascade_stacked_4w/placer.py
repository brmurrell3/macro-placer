"""CDLNSSACascadeStacked4WPlacer — cascade_saddle FOLLOWED BY portfolio_saddle.
...
class CDLNSSACascadeStacked4WPlacer:
    ...
    def __init__(
        ...
        # Four non-canonical weights — adds equal-weighted (1,1,1) to the
        # 3-weight portfolio. Canonical (1, 0.5, 0.5) handled by cascade.
        self.portfolio = [
            (1.0, 0.0, 1.0),  # cong-focus
            (1.0, 1.0, 0.0),  # density-focus
            (0.0, 1.0, 1.0),  # non-WL
            (1.0, 1.0, 1.0),  # equal-weight (new — symmetric in all 3)
        ]

    def place(self, benchmark: Benchmark) -> torch.Tensor:
        log = lambda s: print(s, flush=True) if self.verbose else None
        log(f"=== CDLNSSACascadeStacked4WPlacer ({benchmark.name}) ===")
        t0 = time.time()
