...
_STACKED_PATH = _ROOT / "submissions" / "cd_lns_sa_cascade_stacked_finalsa" / "placer.py"
_spec = importlib.util.spec_from_file_location("stacked_placer_finalsa", str(_STACKED_PATH))
_stacked = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_stacked)
CDLNSSACascadeStackedFinalSAPlacer = _stacked.CDLNSSACascadeStackedFinalSAPlacer
...
class CDLNSSACascadeStackedPeripheryFinalSAPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeStackedPeripheryFinalSAPlacer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        stacked = CDLNSSACascadeStackedFinalSAPlacer(
