"""CDLNSSACascadeStackedFinalSAV2Placer — cascade_saddle FOLLOWED BY portfolio_saddle.
...
class CDLNSSACascadeStackedFinalSAV2Placer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeStackedFinalSAV2Placer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        if self.budget_seconds is not None:
            ...
            # finalsa_v2: trim each phase by ~3-5% to guarantee Phase 4 SA budget
            e25_kwargs = dict(
                cd_hard_cap_s=B * 0.17, lns_budget_s=B * 0.05, sa_budget_s=B * 0.05,
            )
            e41_kwargs = dict(
                cd_hard_cap_s=B * 0.17, lns_budget_s=B * 0.04, sa_budget_s=B * 0.04,
                kjoint_budget_s=B * 0.04,
            )
            cascade_budget_target = B * 0.17  # ~561s instead of 660s
            portfolio_budget_target = B * 0.10  # ~330s instead of 429s
            # ↑ trim saves 0.16 B = 528s → Phase 4 SA gets ~500s consistently
        ...
        # Phase 4: final SA-v2 polish with HIGHER T₀ to escape any residual local minima.
        # Uses budget remaining after standard pipeline; safe — best-so-far tracking
        # restores input placement if no improvement found. Falls back gracefully.
        if deadline is not None:
        ...
        if remaining > 30.0:
            try:
                ...
                _e25_path = _ROOT / "submissions" / "cd_lns_sa" / "placer.py"
                _e25_spec = importlib.util.spec_from_file_location("e25_placer_finalsa", str(_e25_path))
                _e25_mod = importlib.util.module_from_spec(_e25_spec)
                _e25_spec.loader.exec_module(_e25_mod)
                ...
                evaluator = IncrementalProxyEvaluator(benchmark, plc, best_placement.clone())
                log(f"  Phase 4: final SA-v2 (budget {remaining:.0f}s, T₀=1e-3, hi-T)")
                sa_stats = run_sa_polish_v2(
                    evaluator, benchmark, plc, hard_movable,
                    time_budget_s=remaining,
                    T0=1e-3, Tf=1e-6,
                    seed=43,  # different from E25's seed=42 to explore new chain
                    log_fn=None,
                )
                final_placement = evaluator.placement.detach().clone().to(torch.float32)
                final_proxy = float(compute_proxy_cost(final_placement, benchmark, plc)["proxy_cost"])
                final_ovl = compute_overlap_metrics(final_placement, benchmark)["overlap_count"]
                if final_ovl == 0 and final_proxy < best_proxy - 1e-7:
                    log(f"  final SA-v2: {final_proxy:.5f} (Δ={final_proxy - best_proxy:+.5f}) ACCEPT")
                    best_proxy = final_proxy
                    best_placement = final_placement
                    best_name = "final_sa"
                else:
                    log(f"  final SA-v2: {final_proxy:.5f} ovl={final_ovl} REJECT")
            except Exception as exc:
                log(f"  final SA-v2 skipped: {exc}")
