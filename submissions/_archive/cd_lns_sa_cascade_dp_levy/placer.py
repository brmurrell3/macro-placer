"""CDLNSSACascadeDPLevyPlacer — DP-lane hybrid + Lévy-flight cascade saddle.

Composition of two independently-validated wins:
  1. `submissions/cd_lns_sa_cascade_dp_lane/placer.py` (E91 PATH B; --all 1.0669)
     — three-lane init (E25 + E41 + DP) with full polish on each, best-of-3
     plateau pick before saddle.
  2. `experiments/E97_levy_saddle/code/levy_saddle.py` (E97 PATH C/2;
     +0.13-0.23% on top of cascade saddle across ibm01/03/10) — heavy-tail
     (half-Cauchy) ε magnitudes instead of fixed `(0.3, 1.0, 3.0)` grid.

This file copies the structure of the DP-lane placer verbatim, replacing
only the saddle escape call: `cascading_saddle_escape` → `levy_saddle_escape`.
All budget allocations, lane gating, fallback chains preserved.

Expected aggregate lift vs 1.0669 baseline: 0.1-0.2 % (saddle phase is one
of several phases; Lévy improvement scales with saddle's share of budget).

Cloud setup (same as parent):
...
class CDLNSSACascadeDPLevyPlacer:
    """Cascade DP-lane hybrid with Lévy-flight saddle escape (composition)."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeDPLevyPlacer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        if deadline is not None:
            ...
            if remaining < 400.0:
                ...
                log(f"  Phase 5: Lévy cascading saddle (budget {cascade_budget:.0f}s, "
        ...
        all_candidates = [(e25_proxy, e25, "E25"),
                          (cascade_proxy, cascade_state, "levy_cascade")]
        if e41 is not None:
