...
_STACKED_PATH = _ROOT / "submissions" / "cd_lns_sa_cascade_stacked_dualseed" / "placer.py"
_spec = importlib.util.spec_from_file_location("stacked_placer_dualseed", str(_STACKED_PATH))
_stacked = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_stacked)
CDLNSSACascadeStackedDualseedPlacer = _stacked.CDLNSSACascadeStackedDualseedPlacer
...
class CDLNSSACascadeStackedPeripheryDualseedPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeStackedPeripheryDualseedPlacer ({benchmark.name}) ===")
        t0 = time.time()
        ...
        stacked = CDLNSSACascadeStackedDualseedPlacer(
