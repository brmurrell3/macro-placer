...
# Reuse the Lévy cascade placer.
_LEVY_PATH = _ROOT / "submissions" / "cd_lns_sa_cascade_levy" / "placer.py"
_spec = importlib.util.spec_from_file_location("levy_placer", str(_LEVY_PATH))
_levy = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_levy)
CDLNSSACascadeLevyPlacer = _levy.CDLNSSACascadeLevyPlacer
...
class CDLNSSACascadeLevyPeripheryPlacer:
    """Lévy cascade + periphery-bias post-pass (strictly conservative)."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeLevyPeripheryPlacer ({benchmark.name}) ===")
        t0 = time.time()
