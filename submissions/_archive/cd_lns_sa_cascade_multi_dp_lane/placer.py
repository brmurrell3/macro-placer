"""CDLNSSACascadeMultiDPLanePlacer — cascade + multi-config DP basin selector.

H4a (2026-05-16): identical to `cd_lns_sa_cascade_dp_lane/placer.py` except
the DP lane runs K=4 DP configs (via E96 multi_dp_basin) and picks the
best raw basin by `legalize_proxy + 10×residual_overlap`, then polishes
that winner with the same budget as the previous single-DP lane.

Cached oracle (memory: cached_best_aggregate_1051) shows multi_DP wins
ibm14 by −7.7% vs single-config DP — selecting across K=4 configs is
worth ~+0.4-0.5% aggregate IBM (ibm14 win × 1/17 benches).

Wall overhead: K configs × ~30-60s each minus the single-DP run we
replaced. On large benches K=4 adds ~90-150s; budget for cascade
saddle preserved by tightening polish budget by 0.01×B.

Graceful fallback: if `DREAMPLACE_ROOT` is not set, behaves identically
to `cd_lns_sa_cascade/placer.py`.
"""
...
# Multi-config DP basin selector (E96). Picks the best raw basin across K
# configs by legalize_proxy + 10×residual_overlap. Drop-in replacement for
# single-DP `_try_run_dreamplace`.
...
class CDLNSSACascadeMultiDPLanePlacer:
    """Cascade with multi-config DP basin selector as third lane.

    H4a (2026-05-16): replaces the single-DP lane in CDLNSSACascadeDPLanePlacer
    with K-config DP basin selection (E96 multi_dp_basin). Same polish budget;
    only the basin selection step differs.
    """
    ...
    def __init__(
        ...
        self.multi_dp_K = int(multi_dp_K)
        self.verbose = verbose

    def place(self, benchmark: Benchmark) -> torch.Tensor:
        log = lambda s: print(s, flush=True) if self.verbose else None
        log(f"=== CDLNSSACascadeMultiDPLanePlacer K={self.multi_dp_K} ({benchmark.name}) ===")
        t0 = time.time()
        ...
        if deadline is not None and (deadline - time.time()) < dp_skip_threshold:
            ...
            log(f"  Phase 3: multi-DP basin selector (K={self.multi_dp_K}) + full polish")
            multi_dp_deadline = (deadline - self.budget_seconds * 0.20) if deadline else None
            # multi-DP needs to complete ~0.20×B before its polish budget begins,
            # so its own deadline is the overall deadline minus polish budget.
            dp_raw, _legal, dp_stats = multi_dp_basin(
                benchmark, plc, K=self.multi_dp_K,
                deadline=multi_dp_deadline, log=log,
            )
            if dp_raw is not None:
                log(f"  multi-DP winner: {dp_stats.get('winner_label', '?')} "
                    f"(legal_proxy={dp_stats.get('winner_legalize_proxy', float('nan')):.4f})")
                ...
                log(f"  multi-DP returned no basin; DP lane skipped")
