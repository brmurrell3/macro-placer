...
class CDLNSSACascadeNoE41DeepPlacer:
    """Variant: SKIP E41 lane; reallocate its 0.32×B to cascade + portfolio.

    Hypothesis: portfolio wins 17/17 in champion run — E41 plateau is just
    a starting point for portfolio. Skipping E41 saves ~17min, doubling
    cascade (0.20→0.36) and portfolio (0.13→0.29) budgets. More saddle
    iters may find deeper basins, outweighing the worse-starting-plateau
    cost.
    """
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        if self.budget_seconds is not None:
            ...
            # NO E41 LANE — reclaim E41's 0.32×B for cascade + portfolio.
            e25_kwargs = dict(
                cd_hard_cap_s=B * 0.20, lns_budget_s=B * 0.06, sa_budget_s=B * 0.06,
            )
            e41_kwargs = {}
            cascade_budget_target = B * 0.36  # was 0.20, +0.16 from E41 trim
            portfolio_budget_target = B * 0.29  # was 0.13, +0.16 from E41 trim
            log(f"  budget {self.budget_seconds:.0f}s (NO E41); "
        ...
        try:
            ...
            log(f"  E25 FAILED ({exc})")

        # SKIPPED Phase 2 (E41) in this variant.
        e41 = None
        e41_proxy = float("inf")

        if e25 is None and e41 is None:
            raise RuntimeError("E25 failed and E41 skipped; cannot proceed")
