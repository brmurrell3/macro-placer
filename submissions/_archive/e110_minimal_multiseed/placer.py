"""E110MinimalMultiseed — N-seed ensemble of E110Minimal, pick best.

Runs N independent E110 + CD polish attempts with different seeds.
Returns the placement with lowest canonical proxy. Each run starts
from a different random perturbation of SDF init.

If single-seed E110Minimal at 300s gives quality X, the K-seed
ensemble at 300s/K each should approach the BEST basin among K
samples. Cheap variance reduction.

Default N=4 with full budget (3300s / 4 = 825s per seed). Wall is
similar to a single full cascade but might find a deeper basin.
"""
...
class E110MinimalMultiseedPlacer:
    """N-seed ensemble of E110+CD; pick best basin."""
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        self._log(f"=== E110MinimalMultiseed ({benchmark.name}), N={self.n_seeds} ===")

        bench_dir = find_benchmark_dir(benchmark.name)
        _, plc = load_benchmark_from_dir(str(bench_dir))

        # Divide budget; reserve ~60s overhead per seed.
        per_seed_total = self.budget_seconds / self.n_seeds
        per_seed_cd = max(120.0, per_seed_total - 90.0)

        best_pos, best_proxy, best_seed = None, float("inf"), None
        for i in range(self.n_seeds):
            seed = 42 + i * 1000
            if deadline is not None and (deadline - time.time()) < per_seed_total * 0.5:
        ...
        if best_pos is None:
            raise RuntimeError("All multiseed runs failed or produced overlaps")
