...
# Tabu cascade saddle escape from E99 (orthogonalized eigvecs + Lévy magnitudes).
_E99_CODE = _ROOT / "experiments" / "E99_tabu_cascade" / "code"
if str(_E99_CODE) not in sys.path:
    sys.path.insert(0, str(_E99_CODE))
from tabu_cascade import tabu_levy_saddle_escape
...
class CDLNSSACascadeTabuStackedPlacer:
    """Tabu cascade (canonical, orthogonalized eigvecs) + portfolio (3 weights).

    Variant of E109 stacked: replaces E84 cascade with E99 tabu_levy_saddle,
    which forces each cascade iter to pick an eigvec orthogonal to recently-
    used ones. Avoids revisiting the same soft direction, increasing
    DIRECTION diversity beyond what single-vec cascade does. Composes with
    portfolio_saddle's WEIGHT diversity for complementary exploration.
    """
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        if cascade_budget < 60.0:
            log(f"  Phase 3a: tabu cascade SKIPPED (budget {cascade_budget:.0f}s)")
        else:
            log(f"  Phase 3a: tabu_levy_saddle canonical (budget {cascade_budget:.0f}s, "
                f"max_iters={self.cascade_max_iters})")
            try:
                cascade_state, stats = tabu_levy_saddle_escape(
                ...
                log(f"  tabu cascade done: {cascade_proxy:.5f} iters={stats.get('iters_run', '?')}")
            except Exception as exc:
                log(f"  tabu cascade failed: {exc}; falling back to plateau")
                cascade_state = plateau
                cascade_proxy = plateau_proxy
