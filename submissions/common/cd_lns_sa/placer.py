"""E25 — CDAdaptive + grid-bin LNS + SA polish v2 (champion candidate, NOT promoted).

Verified avg --all 1.0954 on 2026-04-29 (-0.33% vs E12 champion 1.0990,
-1.95% vs leaderboard 1.1172, zero overlaps). Champion remains E12
CDLNSGridBin; this code stays here as a *candidate* awaiting human
decision. ADR-008 status: Proposed (not Accepted). Do not cite as
champion.

The pipeline is *compositional*: CD plateau, then LNS escape via
grid-bin moves (different move type than CD's per-axis breakpoints),
then SA-v2 polish on per-axis breakpoints with best-so-far tracking
and low T₀.
...
Pipeline (per benchmark, total ≤ 3600 s legal cap):
  ...
  5. LNS phase (≤ 600 s, grid-bin destroy/reinsert, cost-aware destroy).
     Different candidate set than CD's per-axis breakpoints — escapes CD's
     per-axis fixed point.
...
The compositional gain over E12 (CD + LNS only): −0.33 % on average across
17 IBM benchmarks. SA-after-LNS finds wins on the easier benchmarks
(ibm01: −1.58 %, ibm10: −0.97 %, ibm08: −0.69 %) that E12 LNS-alone left
on the table; the hardest benchmarks (ibm12, ibm13, ibm14, ibm15, ibm17)
saw zero or sub-noise SA lift after LNS — those plateaus are robust to
both move types.

All hyperparameters are global. No per-benchmark tuning.

References:
- E14 SA polish — falsified due to no best-so-far tracking + T₀ too high.
- E24 SA polish v2 — fixed implementation; matched E12-LNS on `--fast`.
- E25 (this) — composes E24's SA-v2 with E12's LNS overlay.
- ADR-008 — promotion decision.
"""
...
# ── Grid-bin LNS primitives (inlined from E12 cd_lns_gridbin) ──────────────
...
# ── SA polish v2 primitive (inlined from E24 cd_sa_polish_v2) ──────────────
...
def run_sa_polish_v2(
    ...
    Move set: per-axis breakpoints from cd_core.axis_breakpoints — same as
    CD's. Acceptance: Metropolis with geometric cooling on elapsed/budget.

    Best-so-far: tracks ``best_proxy`` and ``best_placement`` across the
    chain. On every accepted move, if cur_proxy < best, snapshot the
    placement. At budget exhaustion, restore evaluator to best by walking
    chain-final → best via per-macro move() calls (keeps the V/H congestion
    cache in sync — direct mutation of evaluator.placement would desync).
    """
...
class CDLNSSAPlacer:
    """E25 placer (champion candidate at avg --all 1.0954, NOT promoted): CD + LNS + SA-v2.
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        self._log(
            f"=== CDLNSSAPlacer ({benchmark.name}): "
