"""CDLNSSACascadeWMSeedEnsemblePlacer — N parallel Option-C runs, best-of-N.
...
Architecture:
  ...
  2. Each child:
       ...
       d. Runs CDLNSSACascadeStackedWireMaskPeripheryPlacer with the given seed.
...
def _worker_entry(seed_id: int, bench_name: str, budget_seconds: float,
    ...
    try:
        ...
        opt_c_spec = importlib.util.spec_from_file_location(
            "opt_c_p",
            str(_ROOT / "submissions" / "cd_lns_sa_cascade_stacked_wiremask_periphery" / "placer.py"),
        )
        ...
        placer = opt_c_mod.CDLNSSACascadeStackedWireMaskPeripheryPlacer(
...
class CDLNSSACascadeWMSeedEnsemblePlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        log(f"=== CDLNSSACascadeWMSeedEnsemblePlacer ({benchmark.name}) "
...
Placer = CDLNSSACascadeWMSeedEnsemblePlacer
