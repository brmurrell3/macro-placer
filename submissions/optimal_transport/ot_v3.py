"""
Optimal Transport Placer — v3

Fix: make sliced Wasserstein fully differentiable by expanding macros into
multiple sample points (proportional to area) so we can use equal-weight
sort-and-match 1D OT, which IS differentiable through the positions.

Hybrid with SDF density (from v5) as baseline.
"""
...
def expand_macros_to_points(pos, sizes, total_points=256):
    """Expand macro centers into multiple points proportional to area.

    Each macro gets ceil(area_fraction * total_points) sample points,
    all at the macro center. This makes the OT loss area-weighted
    while keeping equal-weight sort-and-match OT differentiable.
    """
    areas = sizes[:, 0] * sizes[:, 1]
    fracs = areas / areas.sum()
    counts = (fracs * total_points).clamp(min=1).long()
    # Adjust to exactly total_points
    diff = total_points - counts.sum().item()
    if diff > 0:
        _, topk = torch.topk(fracs, min(diff, len(fracs)))
        counts[topk] += 1
    elif diff < 0:
        _, topk = torch.topk(fracs, min(-diff, len(fracs)), largest=False)
        counts[topk] = (counts[topk] - 1).clamp(min=1)

    # Expand: repeat each position by its count
    expanded = torch.repeat_interleave(pos, counts, dim=0)
    return expanded, counts


def sliced_wasserstein_sortmatch(source_pts, target_pts, n_slices=16):
    """Differentiable sliced Wasserstein-2 via sort-and-match.

    Both point clouds must have the same size. Sort projections onto
    random 1D directions and compute squared distance between matched pairs.
    Differentiable through source_pts via soft sorting.
    """
    ...
    angles = torch.linspace(0, math.pi, n_slices + 1, device=source_pts.device)[:-1]
    directions = torch.stack([torch.cos(angles), torch.sin(angles)], dim=1)  # [S, 2]

    # Project: [N, S]
    src_proj = source_pts @ directions.T
    tgt_proj = target_pts @ directions.T

    total_cost = torch.tensor(0.0, device=source_pts.device)

    for s in range(n_slices):
        # Sort source by projection
        src_sorted, src_idx = torch.sort(src_proj[:, s])
        # Sort target by projection (detached since target is fixed)
        tgt_sorted, _ = torch.sort(tgt_proj[:, s])

        # Match sorted positions — this IS differentiable through src_sorted
        total_cost = total_cost + ((src_sorted - tgt_sorted) ** 2).mean()
...
class OTPlacer:
    """Hybrid OT + SDF density placer v3 — differentiable sliced Wasserstein."""

    def __init__(self, seed=42, n_iters=500, lr=3.0,
                 density_weight=0.4, overlap_weight=80.0,
                 grid_resolution=32, ot_weight=0.5, n_slices=16,
                 ot_points=256):
        self.seed = seed
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Create uniform target points for OT (fixed, detached)
        ...
        # Precompute macro point counts (proportional to area)
        areas = sizes[:, 0] * sizes[:, 1]
        fracs = areas / areas.sum()
        ...
        for step in range(self.n_iters):
            ...
            # OT spreading via sliced Wasserstein
            expanded_pts = torch.repeat_interleave(clamped, counts, dim=0)
            ot_loss = sliced_wasserstein_sortmatch(
                expanded_pts, target_pts, self.n_slices)
            ...
            # Weights: SDF density fades, OT fades, WL stays
            dw = self.density_weight * (1.5 - 0.8 * frac)
            # OT strong early for global spreading, off in second half
            ot_w = self.ot_weight * max(0, 1.0 - frac * 2.0)
