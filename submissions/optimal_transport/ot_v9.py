"""
Optimal Transport Placer — v9

Key fixes:
1. Include density/OT in best_pos selection (not just WL + overlap)
2. Print diagnostics about position movement
3. Use Sinkhorn with aggressive parameters
4. Combine SDF density + OT for best of both worlds
"""
...
class OTPlacer:
    """OT + SDF density placer v9 — includes density in checkpoint selection."""

    def __init__(self, seed=42, n_iters=500, lr=3.0,
                 density_weight=0.4, ot_weight=100.0,
                 overlap_weight=80.0,
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        for step in range(self.n_iters):
            ...
            # OT (every step now that we use small parameters)
            ...
            with torch.no_grad():
                if step % 20 == 0 or step == self.n_iters - 1:
                    ...
                    # Include density in selection metric (proxy cost formula)
                    cost = wl_loss.item() + 0.5 * density_loss.item() / 10.0 + 0.1 * ov_val / (cw * ch)
                    if cost < best_cost:
                        best_cost = cost
        ...
        avg_move = (dist * moved[:n_hard]).sum() / moved[:n_hard].sum()
        print(f"  [diag] avg_move={avg_move.item():.1f}μm max_move={dist.max().item():.1f}μm")
        ...
        print(f"  [diag] legalize avg_shift={legal_dist.mean():.1f}μm max_shift={legal_dist.max():.1f}μm")
