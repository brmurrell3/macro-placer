"""
Optimal Transport Placer — v7

Use geomloss Sinkhorn on the SDF density grid vs uniform target.
This is the proper OT formulation: transport the actual density field
to uniform density, not just point positions.

Key insight: compute SDF density on grid, convert to weighted point cloud,
then use Sinkhorn divergence. Only compute OT every K steps for speed.
"""
...
class OTPlacer:
    """OT placer v7 — Sinkhorn on density grid."""

    def __init__(self, seed=42, n_iters=500, lr=3.0,
                 density_weight=0.4, ot_weight=2.0,
                 overlap_weight=80.0,
                 grid_resolution=32, ot_every=5):
        self.seed = seed
        ...
        self.ot_every = ot_every  # compute OT loss every N steps
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Grid coordinates for OT — each grid cell is a weighted point
        gx_2d, gy_2d = torch.meshgrid(grid_x, grid_y, indexing='xy')
        grid_coords = torch.stack([gx_2d.flatten(), gy_2d.flatten()], dim=1)  # [M, 2]
        n_grid = grid_coords.shape[0]

        # Uniform target weights (normalized)
        uniform_weights = torch.ones(n_grid) / n_grid

        # Sinkhorn loss for density grids
        ot_loss_fn = SamplesLoss(
            loss="sinkhorn", p=2,
            blur=canvas_diag * 0.02,  # small blur for sharp OT
            scaling=0.8,
            debias=True,
        )
        ...
        for step in range(self.n_iters):
            ...
            # Top-10% density penalty
            k_density = max(1, int(0.10 * len(flat_density)))
            top_density, _ = torch.topk(flat_density, k_density)
            density_loss = top_density.mean()

            # OT: Sinkhorn divergence between density-weighted grid and uniform
            compute_ot = step % self.ot_every == 0 and frac < 0.7
            if compute_ot:
                # Normalize density to get source weights
                density_sum = flat_density.sum()
                if density_sum > 0:
                    source_weights = flat_density / density_sum
                else:
                    source_weights = uniform_weights
                # Sinkhorn divergence
                ot_loss = ot_loss_fn(source_weights, grid_coords, uniform_weights, grid_coords)
            else:
            ...
            if step == 0:
                print(f"  [debug] wl={wl_loss.item():.3f} den={density_loss.item():.1f} "
                      f"den_contrib={dw * density_loss.item():.1f} "
                      f"ot={ot_loss.item():.3f} ot_contrib={ot_w * ot_loss.item() / (canvas_diag**2):.4f}")
