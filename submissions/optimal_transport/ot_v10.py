"""
Optimal Transport Placer — v10

Key insight: the gradient optimizer does nothing because initial positions
have overlaps that trap it. Solution: legalize FIRST, then optimize the
legalized (overlap-free) positions.

Use OT spreading + WL to refine the legalized layout, with strict overlap
penalty to maintain feasibility.
"""
...
class OTPlacer:
    """OT placer v10 — legalize first, then optimize."""

    def __init__(self, seed=42, n_iters=300, lr=0.5,
                 density_weight=0.4, ot_weight=50.0,
                 overlap_weight=200.0,
                 grid_resolution=32, blur=0.01, scaling=0.5):
        self.seed = seed
        self.n_iters = n_iters
        self.lr = lr  # smaller LR for refinement
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Step 2: Optimize from legalized positions
        ...
        for step in range(self.n_iters):
            ...
            # Overlap — very strong since we start overlap-free
            ...
            loss = wl_loss + dw * density_loss + ot_w * ot_loss + self.overlap_weight * ov_loss
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            with torch.no_grad():
                if step % 10 == 0 or step == self.n_iters - 1:
                    ...
                    # Multi-objective selection: WL + density + overlap
                    cost = wl_loss.item() + 0.3 * density_loss.item() + 100 * ov_val / (cw * ch)
                    if cost < best_cost:
                        best_cost = cost
        ...
        avg_move = (dist * moved[:n_hard]).sum() / max(moved[:n_hard].sum(), 1)
        print(f"  [diag] avg_move={avg_move.item():.2f}μm max_move={dist.max().item():.2f}μm")

        # Final legalization (should be minimal if overlap penalty worked)
        ...
        legal_dist = np.sqrt(((legal_pos - pos_np) ** 2).sum(axis=1))
        print(f"  [diag] legalize avg_shift={legal_dist.mean():.2f}μm max_shift={legal_dist.max():.2f}μm")
