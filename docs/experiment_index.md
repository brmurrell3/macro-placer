...
| ~~E61~~ | ~~Genetic-algorithm crossover between E25 and E41 outputs (V1 per-macro Bernoulli; V2 spatial-block 2×2 quadrants); polish via CD+LNS+SA-v2~~ | `experiments/E61_ga_crossover/code/cd_lns_ga_crossover.py`, manifest | V1 ibm01: falsified. V2 ibm12 smoke 1.19898 (−0.61 %). V2 --fast 0.91342 (−0.74 %). V2 --ng45 0.69078, ariane133 0.6760. **V2 --all KILLED at 8/17: subset avg +0.04 % vs E48 (sub-noise tied)**. | smoke→fast→ng45→all | **FALSIFIED 2026-05-02 23:34** — V1 Bernoulli unrecoverable overlaps; V2 spatial-block fast/ng45 lifts traced primarily to DPO seed-noise (ibm09 --fast 0.8259 did NOT replicate at --all 0.8413). **--all 8-bench distribution: 2 wins, 2 ties, 4 losses around E48** — pattern is sub-noise drift, not systematic improvement. **Lesson: recombining existing basins doesn't break new ground; need structurally different attack mechanism.** Killed before completing 17 benches; user pivoted to bigger breakthrough hypotheses. |
