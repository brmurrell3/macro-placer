...
| E41 | DPO init + K-joint LNS combo (E18 ⊕ E39) — composes basin shift + joint-move escape | `experiments/E41_dpo_kjoint/code/cd_lns_sa_dpo_kjoint.py`, manifest | **0.92178 (--fast)**, -1.27 % vs E25 fast 0.9336 (strongest --fast tonight); **0.69022 (--ng45)**, -1.91 % vs E12 (strongest NG45 tonight); --all rerun in progress 2026-04-30 05:16 (--jobs 4 after the previous --jobs 8 stall) | fast→ng45→all (rerunning) | **STRONG-CANDIDATE 2026-04-30** — beats both parents on `--fast` AND on NG45. K-joint bug fix verified on E39 ibm07 single-bench 2026-04-30 04:56. --all rerun progress visible in `/tmp/e41_all_postfix.log`; final number will land in `results/experiment_log.jsonl` as `e41_dpo_kjoint_all_postfix`. |
