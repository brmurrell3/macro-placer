...
Differentiates through the actual proxy cost formula:
    f(p) = WL(p) + 0.5 * D(p) + 0.5 * C(p)

using log-sum-exp HPWL, differentiable grid density (top-10%),
and differentiable RUDY congestion (ABU-5%). Overlap managed via
penalty continuation across three phases.

See docs/dpo.md for full theory.
"""
...
class DPOPlacer:
    """Differentiable Proxy Optimization placer."""
